...
import { Menu, X, Shield, ShoppingCart, Heart, Gamepad2, LogIn, ArrowRight } from 'lucide-react';
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-12">
            ...
            <div className="hidden md:block">
              <ul className="flex gap-3 text-zinc-400 md:gap-9">
                {navLinks.map((link) => (
                  <li key={link.name}>
                    <a rel="nofollow" className={navLinkClasses} href={link.href} target={link.external ? '_blank' : undefined}>
                      <span>{link.name}</span>
                      {link.external && <ArrowRight className="block transition-transform group-hover:rotate-45" height="1em" width="1em"/>}
                    </a>
                  </li>
                ))}
              </ul>
          ...
          <div className="flex items-center gap-2">
            {loading ? null : user ? (
              ...
              <>
                 <Button variant="ghost" asChild className="text-zinc-100 transition-all hover:scale-[1.02] hover:bg-zinc-800 hover:text-zinc-50 active:scale-[0.98] rounded-md px-4 py-1 text-base">
                    <Link href="/signup">Sign up</Link>
                </Button>
                 <Button asChild className="rounded-md bg-gradient-to-br from-blue-400 to-blue-700 ring-2 ring-blue-500/50 ring-offset-2 ring-offset-zinc-950 transition-all hover:scale-[1.02] hover:ring-transparent active:scale-[0.98] active:ring-blue-500/70 px-4 py-1 text-base text-zinc-100">
